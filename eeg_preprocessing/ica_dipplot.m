...
% copied from EEGLAB's dipfit dipplot to customize for my gui
...
function [outsources, XX, YY, ZZ, XO, YO, ZO] = ica_dipplot( sourcesori, AXES, varargin )
...
%         figure;   % Samran, also pos1, pos2 and pos3
...
%         axes(AXES.sagittal);  newsources = ica_dipplot(sourcesori, AXES, 'view', [1 0 0] , options{:}); axis off; 
%         axes(AXES.transverse); newsources = ica_dipplot(sourcesori, AXES, 'view', [0 0 1] , options{:}); axis off; 
%         axes(AXES.coronal); newsources = ica_dipplot(sourcesori, AXES, 'view', [0 -1 0], options{:}); axis off;
        % Samran 18-jun-2019 05:00 Auckland
        % to increase execution speed
        h_figure = findobj ('tag', 'figure_mark_ICs_gui');
        set (h_figure, 'CurrentAxes', AXES.sagittal);  newsources = ica_dipplot(sourcesori, AXES, 'view', [1 0 0] , options{:}); axis off; 
        set (h_figure, 'CurrentAxes', AXES.transverse); newsources = ica_dipplot(sourcesori, AXES, 'view', [0 0 1] , options{:}); axis off; 
        set (h_figure, 'CurrentAxes', AXES.coronal); newsources = ica_dipplot(sourcesori, AXES, 'view', [0 -1 0], options{:}); axis off; 
...
%         axis off;
    ...
    elseif strcmpi(g.summary, '3d')
        ...
        axes('position', [0   0 1/3 1], 'tag', 'rear'); ica_dipplot(sourcesori, options{:}, 'holdon', 'on'); view([0 -1 0]);
        axes('position', [1/3 0 1/3 1], 'tag', 'top' ); ica_dipplot(sourcesori, options{:}, 'holdon', 'on'); view([0  0 1]);
        axes('position', [2/3 0 1/3 1], 'tag', 'side'); ica_dipplot(sourcesori, options{:}, 'holdon', 'on'); view([1 -0.01 0]);
    ...
    % Samran 23-nov-2017 17:34:
    % commented this to speed up the working of dipplot with my GUI
...
%                    'style', 'text', 'string', '');
    ...
    % set(gcf, 'color', BACKCOLOR);     % Samran 15-nov-2017 21:15:
    % commented this to fix the wrong working of dipplot with my GUI
    
    if strcmp(g.gui, 'off') | strcmpi(g.holdon, 'on')
%         set(findobj('parent', gcf, 'tag', 'tmp'), 'visible', 'off');
        % Samran 23-nov-2017 17:34: commented this to speed up the working
        % of dipplot with my GUI 
        delete(findobj('parent', gcf, 'tag', 'tmp'));
    end;
    if strcmp(g.mesh, 'off')
        set(findobj('parent', gca, 'tag', 'mesh'), 'visible', 'off');
        % Samran 23-nov-2017 17:34: commented this to speed up the working
        % of dipplot with my GUI 
        delete(findobj('parent', gca, 'tag', 'mesh'));
    end;
